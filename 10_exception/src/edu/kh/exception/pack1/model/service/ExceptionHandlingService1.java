...
public class ExceptionHandlingService1 {
	 ...
	 * 
	 * - catch(예외){} : try 구문 시동 중 예외가 발생되어 던져진 경우
	 ...
	 * 1) throw : 예외 강제 발생(던짐)
	 * 2) 예외 + 다형성 
	 * 	- catch문 여러 개 작성 시 부모 예외를 처리하는
	 * 		catch문이 자식 예외도 모두 잡아서 처리
...
//			}
			...
			// Unreachable catch block : 도달할 수 없는 catch 구문
				...
				System.out.println("----- 참조하는 객체가 없는데 참조를 시도 처리 완료 -----");
			}
