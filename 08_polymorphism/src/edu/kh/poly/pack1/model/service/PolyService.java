...
//		s2.setAndroidVersion(15); // 오류
	 ...
	 *  	부모 타입 참조 변수를 자식 타입으로 강제 형변환 하여
...
//		int v2 = s2.getAndroidVersion();
		...
		// (참조 변수가 자식 타입으로 벼해서 자식 객체 참조 가능)
		int v1 = ((Iphone)s1).getIosVersion();
		int v2 = ((Galaxy)s2).getAndroidVersion();
		

		// 다운 캐스팅 + 얕은 복사(주소만 복사)
		Iphone i1 = (Iphone)s1; // s1을 Iphone으로 강제 형변화 후 주소를 i1에 대입
		Galaxy g1 = (Galaxy)s2;
	 ...
	 * 	 호출되는 메서드가 어떤 클래스의 메서드와 연결되는지 결정
	 */
	public void method3() {
	...
	public void method4() {
		...
		// s1.toString() 작성 시 SmartPhone의 toString() 호출된다고 표시됨
		// == 정적 바인딩
		//(참조 변수가 부모 타입이니까 부모 부분의 메서드 호출)
		
		// 실행을 해보니 SmartPhone.toString()이 아닌
	...
	public void method5() {
		...
		// 객체배열에 다형성을 적용함으로써 배열 하나로 다양한 자료형을
		// 묶음으로 다룰 수 있게 됨
		// -> 변수 생성 코드 감소 + 반복문으로 한 번에 저어 가능
		// --> 전반적으로 코드 길이 감소
	...
	public void printToString(SmartPhone s) {  
		...
		// 실제 참조하는 객체가 자식 객체면 자식 객체의 오버라이딩 된 toString() 호출(동적 바인딩)
		System.out.println(s.toString());
	}
...
//	}
	...
	public void method8() {
		...
		System.out.print("s1 instanceof Iphone : ");
		System.out.println(s1 instanceof Iphone); // true
	}
