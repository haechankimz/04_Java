...
0. 프로그램 종료					
	...
	private Student createStudent() {
		...
		return new Student(name, studentNumber, gender, java, html);
	}
	...
	private String studentInform(Student s) {
		...
		return String.format("%s / %s / %c / %s / %d / %d",
				Student.schoolName, s.getStudentNumber(), s.getGender(),
	...
	private void updateStudentName(Student s) {
		...
		String beforeName = s.getName(); // 이전 이
	...
	private void updateJava(Student s) {
		...
		System.out.print("수정할 학생 이름 : ");
		int javaPoint = sc.nextInt();
		
		int beforeJavaPoint = s.getJava();
		s.setJava(javaPoint);
		int calc = javaPoint - beforeJavaPoint;
		
		if(javaPoint > beforeJavaPoint) {
			System.out.printf("%d -> %d (%d) \n", beforeJavaPoint, javaPoint, calc);
		}else {
			System.out.printf("%d -> %d (%d) \n", beforeJavaPoint, javaPoint, calc);
		}
