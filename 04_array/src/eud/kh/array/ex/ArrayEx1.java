...
public class ArrayEx1 {
	...
	public void method1() {
		...
		// 배열 선언
		int[] arr;
		// Stack 영역에 int[] 자료형을 참조할 주소를 저장할 변수를 생성하고(주소저장)
		// 그 변수의 이름을 "arr"으로 지정
	...
	/**
	 * 길이를 입력받아 입력받은 길이 만큼의 배열을 생성(할당) 한 후
	...
	public void method4() {
		...
		//입력받은 길이 만큼의 배열을 생성(할당) 한 후
		int[] randoms = new int[len];
		
		//모든 인덱스 요소에 0부터 배열길이 미만의 난수를 대입
		for(int i = 0 ; i < len ; i++) {
			
			randoms[i] = (int)( Math.random() * len); //double -> int 강제 형변환
																								// 데이터 손실을 이용한 소수점 제거
		}
		
		// Arrays.toString(배열참조변수)
		// -> 배열에 모든 요소를 [a,b,c] 형태 문자열로 반환
		System.out.println(randoms); // 주소(비슷한 것)
